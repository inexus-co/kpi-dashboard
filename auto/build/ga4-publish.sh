#!/usr/bin/env bash
# まなんでパズル Webアクセス分析(GA4): 公開フェーズ（AI寸評反映→内側HTML描画→暗号化→履歴再封緘→push）。
...
node $B/encrypt-wrap.js "$C/ga4-inner.html" web-analytics.html "$DASHBOARD_PASSWORD" "まなんでパズル Webアクセス分析ダッシュボード（要パスワード）"
...
echo "[4/4] push"
bash $B/git-push-retry.sh "Update まなんでパズル Webアクセス分析 dashboard (${DNOW})" web-analytics.html auto/cache-ga4.enc
